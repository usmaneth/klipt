...
interface VideoPlaybackProps {
	...
	webcamShape?: "circle" | "rounded-rect" | "square";
	webcamSize?: number;
...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
	(
		...
		useEffect(() => {
			...
			// Apply playback rate immediately when speed regions change (not just in RAF loop)
			const video = videoRef.current;
			if (video) {
				const currentTimeMs = video.currentTime * 1000;
				const active = speedRegions.find(
					(r) => currentTimeMs >= r.startMs && currentTimeMs < r.endMs,
				);
				video.playbackRate = active ? active.speed : 1;
			}
		}, [speedRegions]);
		...
		useEffect(() => {
			...
			// Skip reset if a zoom animation is actively in-flight to avoid snapping
			const currentProgress = animationStateRef.current.progress;
			...
			// Reset motion blur state for clean transitions (only when no zoom in-flight)
			if (currentProgress <= 0 || currentProgress >= 1) {
		...
		useEffect(() => {
			...
			if (app.renderer?.resolution != null) {
		...
		useEffect(() => {
			...
			const ticker = () => {
				...
				// Reset motion blur state when transitioning out of a zoom region
				// to prevent stale blur carrying over to the next zoom
				if (prevZoomProgress > 0 && targetProgress === 0) {
					motionBlurStateRef.current = createMotionBlurState();
				}
				prevZoomProgress = targetProgress;
		...
		useEffect(() => {
			...
			overlay.reset();
		}, [cursorSize, cursorSmoothing, cursorMotionBlur, cursorClickBounce, cursorSway]);
		...
		return (
			<div
				className="relative rounded-xl overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
				style={{
					width: "100%",
			...
			>
				...
				<div className="absolute inset-0 z-0 bg-[radial-gradient(#ffffff22_1px,transparent_1px)] [background-size:16px_16px] pointer-events-none opacity-20" />
				{/* Vignette */}
				<div className="absolute inset-0 z-50 pointer-events-none shadow-[inset_0_0_120px_rgba(0,0,0,0.7)]" />
					...
					style={{
						filter:
							showShadow && shadowIntensity > 0
								? `drop-shadow(0 ${shadowIntensity * 8}px ${shadowIntensity * 32}px rgba(0,0,0,${shadowIntensity * 0.6}))`
								: "none",
					}}
				/>
				...
				{pixiReady && videoReady && (
					<div
						ref={overlayRef}
						className="absolute inset-0 select-none z-30"
					...
					>
						...
						{webcamPath && onWebcamPositionChange && (
							<WebcamOverlay
				...
				<video
					...
					onError={(e) => {
						...
						console.error(`[VideoPlayback] Failed to load video: code=${code} msg=${msg} src=${videoPath}`);
						onError(`Failed to load video (error ${code})`);
					}}
				/>
			</div>
		);
