...
function PlaybackCursor({
	...
	return (
		...
		>
			<div
				className="absolute top-0 bottom-0 w-[2px] bg-white cursor-ew-resize pointer-events-auto"
				style={{
...
function TimelineAxis({
	...
	return (
		<div
			className="h-6 bg-transparent border-b border-white/[0.03] relative overflow-hidden select-none"
			style={{
		...
		>
			...
			{markers.minorTicks.map((time) => {
				...
				return (
					<div
						key={`minor-${time}`}
						className="absolute bottom-0 h-1 w-[1px] bg-white/[0.06]"
				...
				return (
					<div key={marker.time} style={markerStyle}>
						<div className="flex flex-col items-center pb-0.5">
							<div className="h-1.5 w-[1px] bg-white/[0.06] mb-0.5" />
							<span
								className={cn(
									"text-[9px] font-mono tabular-nums tracking-tight",
									marker.time === currentTimeMs ? "text-white/30" : "text-white/15",
								)}
							>
								{marker.label}
							</span>
...
function Timeline({
	...
	return (
		<div
			ref={setRefs}
			style={style}
			className="select-none bg-transparent min-h-[120px] relative cursor-pointer group"
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "Tab" && annotationRegions.length > 0) {
				...
				const overlapping = annotationRegions
					// Use strict < for endMs to match export annotation timing (annotationRenderer.ts)
	...
	return (
		<div className="flex-1 min-h-0 flex flex-col bg-transparent overflow-hidden w-full h-full p-1.5" style={{ perspective: "1500px" }}>
			...
			>
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button
								variant="ghost"
								size="sm"
								className="h-6 px-2 text-[10px] font-mono text-white/20 bg-white/[0.02] border border-white/[0.03] hover:bg-white/[0.05] hover:text-white/40 transition-all duration-150 gap-1 rounded-md"
							>
						...
						<DropdownMenuContent
							align="end"
							className="bg-[#11131A]/95 backdrop-blur-2xl border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.8)] rounded-xl overflow-hidden p-1"
						>
							{ASPECT_RATIOS.map((ratio) => (
								...
								>
									...
									{aspectRatio === ratio && (
										<Check className="w-4 h-4 text-cyan-400 drop-shadow-[0_0_5px_rgba(34,211,238,0.5)]" />
									)}
								</DropdownMenuItem>
							))}
							<div className="mx-2 my-1.5 h-px bg-white/10" />
							<div className="px-3 py-2.5 flex items-center gap-2.5 text-slate-300 bg-white/[0.02] rounded-lg">
								<span className="text-sm font-semibold tracking-wide">Custom</span>
								<input
									type="text"
									inputMode="numeric"
									...
									className="w-12 h-8 rounded-md border border-white/15 bg-black/40 px-2 text-sm text-slate-100 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 shadow-inner"
								...
								<Button
									...
									className="h-8 px-3 text-xs font-bold text-slate-300 hover:text-white hover:bg-white/10 rounded-md"
								>
									Set
								</Button>
								{isCustomAspectRatio(aspectRatio) && (
									<Check className="w-4 h-4 text-cyan-400 drop-shadow-[0_0_5px_rgba(34,211,238,0.5)] ml-auto" />
								)}
							</div>
						</DropdownMenuContent>
					</DropdownMenu>
				...
				<div className="flex items-center gap-2 text-[8px] text-white/10 font-mono tracking-wider uppercase">
					<span className="flex items-center gap-1">
						<kbd className="px-1 py-px bg-white/[0.03] border border-white/[0.05] rounded-[2px] text-white/20 font-mono text-[7px]">
							Side Scroll
						</kbd>
						<span>Pan</span>
					</span>
