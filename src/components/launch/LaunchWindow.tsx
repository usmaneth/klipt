			...
			>
				...
				{showMicControls && (
					<div className={`${styles.electronNoDrag} ${styles.micBarWrapper}`}>
						<div className={`flex items-center gap-2 px-3 py-2 ${styles.micBar}`}>
							<select
								...
								className={`max-w-[200px] rounded-md border border-white/10 bg-white/[0.04] px-2 py-1 text-[10px] text-slate-100 outline-none transition-colors hover:bg-white/[0.07] ${styles.micSelect}`}
							>
								{devices.map((device) => (
				...
				<div className={`${styles.electronDrag} ${styles.hudBarWrapper}`}>
					<div
						className={`mx-auto inline-flex max-w-full items-center gap-1.5 px-3 py-2 ${styles.hudBar}`}
					>
						{/* Drag handle */}
						<div className={`flex items-center ${styles.electronDrag}`}>
							<RxDragHandleDots2 size={12} className="text-white/20" />
						</div>

						{/* Source selector */}
						<button
							type="button"
							className={`inline-flex items-center gap-1.5 px-2 py-1 text-[10px] text-white/70 rounded-lg bg-white/[0.03] transition-colors hover:bg-white/[0.06] disabled:opacity-40 ${styles.electronNoDrag}`}
							onClick={openSourceSelector}
							disabled={recording}
							title={selectedSource}
						>
							<MdMonitor size={12} className="text-white/50" />
						...
						<div className={`flex items-center gap-1 ${styles.electronNoDrag}`}>
							{supportsHudCaptureProtection && (
								...
								>
									{hideHudFromCapture ? (
										<EyeOff size={14} strokeWidth={1.5} className="text-white/25" />
									) : (
										<Eye size={14} strokeWidth={1.5} className="text-[#E0000F]" />
									)}
								</button>
							)}
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									...
									>
										{cameraEnabled ? (
											<Camera size={14} strokeWidth={1.5} className="text-[#22C55E]" />
										) : (
											<CameraOff size={14} strokeWidth={1.5} className="text-white/25" />
										)}
									</button>
								</DropdownMenuTrigger>
										...
										{cameraDevices.length > 1 &&
											cameraDevices.map((device) => (
												<DropdownMenuItem
													...
													className={`cursor-pointer text-xs py-1.5 px-2.5 ${styles.dropdownItem} ${
							...
							>
								{microphoneEnabled ? (
									<MdMic size={14} className="text-[#E0000F]" />
								) : (
									<MdMicOff size={14} className="text-white/25" />
								)}
							</button>
							...
							>
								{systemAudioEnabled ? (
									<MdVolumeUp size={14} className="text-[#E0000F]" />
								) : (
									<MdVolumeOff size={14} className="text-white/25" />
								)}
							</button>
						</div>
						...
						{recording ? (
							<div className={`${styles.recordBtnRecording} ${styles.electronNoDrag}`}>
								...
								<span
									...
									style={{ fontSize: 11, color: "#E0000F" }}
								>
									{formatTime(elapsed)}
								</span>
								<button
							...
							<div className={`inline-flex items-center gap-1.5 px-3 py-1.5 ${styles.electronNoDrag}`}>
						...
						>
							<FolderOpen size={10} className="text-white/15" />
							<span
						...
						<div className={`flex items-center gap-1 ${styles.electronNoDrag}`}>
							...
							>
								<MdVideoFile size={13} className="text-white/15" />
							</button>
							<button
								type="button"
								...
								className={styles.quickActionBtn}
							>
								<FolderOpen size={12} className="text-white/15" />
							</button>
						</div>
							...
							>
								{SUPPORTED_LOCALES.map((code) => (
									<DropdownMenuItem
										...
										className={`text-xs py-1.5 px-2.5 cursor-pointer ${styles.dropdownItem} ${
						...
						<div className={`flex items-center gap-0.5 ${styles.electronNoDrag}`}>
							...
							>
								<FiMinus size={10} className="text-white/12 hover:text-white/30" />
							</button>
							<button
								type="button"
								...
								className={styles.windowControlBtn}
							>
								<FiX size={10} className="text-white/12 hover:text-white/30" />
							</button>
						</div>
					</div>
				</div>
