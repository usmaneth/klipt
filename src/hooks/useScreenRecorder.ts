...
export function useScreenRecorder(): UseScreenRecorderReturn {
  ...
  const preparePermissions = useCallback(async (options: { startup?: boolean } = {}) => {
    ...
    // macOS getMediaAccessStatus('screen') is unreliable for ad-hoc signed apps —
    // it can return "denied" even when the permission is enabled in System Settings.
    // On startup, warn the user if it reports denied so they know to check.
    // During recording, skip this check and let the actual capture attempt succeed or fail.
    if (!options.startup && (!screenPermission.success || screenPermission.status !== "granted")) {
      // Don't block — the permission API is unreliable. Let recording attempt proceed.
      console.warn("Screen recording permission status reported as:", screenPermission.status, "— attempting anyway");
    } else if (options.startup && (!screenPermission.success || screenPermission.status !== "granted")) {
      await window.electronAPI.openScreenRecordingPreferences();
      alert(
        "klipt needs Screen Recording permission before you start. System Settings has been opened. After enabling it, quit and reopen klipt.",
      );
      return false;
    }
    ...
    alert(
      options.startup
        ? "klipt also needs Accessibility permission for cursor tracking. System Settings has been opened. After enabling it, quit and reopen klipt."
        : "Accessibility permission is still missing. System Settings has been opened again. Enable it, then quit and reopen klipt before recording.",
    );
