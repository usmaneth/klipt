...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const preparePermissions = useCallback(async (options: { startup?: boolean } = {}) => {
		...
		if (!options.startup && (!screenPermission.success || screenPermission.status !== "granted")) {
			...
			console.warn(
				"Screen recording permission status reported as:",
				screenPermission.status,
				"— attempting anyway",
			);
		} else if (
			options.startup &&
			(!screenPermission.success || screenPermission.status !== "granted")
		) {
		...
		try {
			...
			if (useNativeMacScreenCapture || useWgcCapture) {
				...
				if (!nativeResult.success) {
					if (useWgcCapture) {
						console.warn(
							"WGC capture failed, falling back to browser capture:",
