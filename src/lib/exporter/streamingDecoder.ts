...
export class StreamingVideoDecoder {
	...
	async loadMetadata(videoUrl: string): Promise<DecodedVideoInfo> {
		...
		if (!videoStream?.width || !videoStream?.height) {
			console.warn(
				`[StreamingDecoder] Video metadata missing dimensions (width: ${videoStream?.width}, height: ${videoStream?.height}) — falling back to 1920x1080`,
			);
		}
	...
	async decodeAll(
		...
		const missingFrames = expectedOutputFrames - exportFrameIndex;
		if (
			!this.cancelled &&
			...
			throw new Error(
				`Video decode ended early at ${lastDecodedFrameSec.toFixed(3)}s (needed ${requiredEndSec.toFixed(3)}s; rendered ${exportFrameIndex}/${expectedOutputFrames} frames, missing ${missingFrames}).`,
			);
		}
	}
