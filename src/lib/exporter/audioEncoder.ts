...
export class AudioProcessor {
	...
	private async renderMixedTimelineAudio(
		...
		if (!("preservesPitch" in media)) {
			console.warn(
				"[AudioProcessor] preservesPitch is not supported on this media element — pitch may shift during speed changes",
			);
		}
	...
	async processEnhancedAudio(enhancedAudioUrl: string, muxer: VideoMuxer): Promise<void> {
		...
		const encoder = new AudioEncoder({
			...
			error: (error: DOMException) =>
				console.error("[AudioProcessor] Enhanced encode error:", error),
		});
		...
		for (let offset = 0; offset < numFrames && !this.cancelled; offset += CHUNK_FRAMES) {
			...
			if (isPlanar) {
				...
				audioBuffer
					.getChannelData(0)
					.subarray(offset, offset + frameCount)
					.forEach((v, i) => {
						data[i] = v;
					});
			}
