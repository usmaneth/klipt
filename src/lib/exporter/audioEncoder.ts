...
export class AudioProcessor {
  ...
  private async renderMixedTimelineAudio(
    ...
    if (!('preservesPitch' in media)) {
      console.warn('[AudioProcessor] preservesPitch is not supported on this media element — pitch may shift during speed changes')
    }
  ...
  /**
   * Process pre-enhanced audio (e.g. Studio Sound WAV) and mux it into the output.
   * Fetches the audio from the given URL, decodes it, then re-encodes as Opus for the muxer.
   */
  async processEnhancedAudio(
    ...
    // Create AudioData frames and encode as Opus
    const encodedChunks: { chunk: EncodedAudioChunk; meta?: EncodedAudioChunkMetadata }[] = []
    const encoder = new AudioEncoder({
      output: (chunk: EncodedAudioChunk, meta?: EncodedAudioChunkMetadata) => {
        encodedChunks.push({ chunk, meta })
      },
      error: (error: DOMException) => console.error('[AudioProcessor] Enhanced encode error:', error),
    })
    ...
    const encodeSupport = await AudioEncoder.isConfigSupported(encodeConfig)
    if (!encodeSupport.supported) {
      console.warn('[AudioProcessor] Opus encoding not supported for enhanced audio, skipping')
      return
    }
    ...
    const CHUNK_FRAMES = 4800 // 100ms chunks
    for (let offset = 0; offset < numFrames && !this.cancelled; offset += CHUNK_FRAMES) {
      const frameCount = Math.min(CHUNK_FRAMES, numFrames - offset)
      const isPlanar = channels > 1
      const format: AudioSampleFormat = isPlanar ? 'f32-planar' : 'f32'

      let data: Float32Array
      if (isPlanar) {
        data = new Float32Array(frameCount * channels)
        for (let ch = 0; ch < channels; ch++) {
          const channelData = audioBuffer.getChannelData(ch)
          data.set(channelData.subarray(offset, offset + frameCount), ch * frameCount)
        }
      } else {
        data = new Float32Array(frameCount)
        audioBuffer.getChannelData(0).subarray(offset, offset + frameCount).forEach((v, i) => {
          data[i] = v
        })
      }
